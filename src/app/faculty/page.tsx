  ...
  return (
    <main className="faculty-page">
              ...
              <div className="faculty-grid">
                {members.map(f => (
                  <article key={f.name} data-tier={f.tier}>
                    <div className="photo"><Image src={f.image} alt={`${f.name}, ${f.subject}`} width={360} height={450} sizes="(max-width: 520px) 96px, (max-width: 800px) 50vw, 25vw" /></div>
      ...
      <style>{`
        ...
        .faculty-grid{grid-template-columns:repeat(4,1fr);gap:8px}.photo{height:132px}.photo img{object-position:center 20%}.copy{padding:10px}.role-badge{padding:3px 6px;margin-bottom:6px;font-size:6px}.copy>span{font-size:6.8px}.copy h3{font-size:18px;margin:4px 0}.copy>b{font-size:7.5px}.copy>p{font-size:8px;line-height:1.45;margin-top:6px;min-height:34px}.faculty-actions{gap:4px;margin-top:9px}.faculty-actions a{padding:7px 4px;font-size:7px}
        @media(max-width:800px){.faculty-grid{grid-template-columns:1fr 1fr}.photo{height:140px}}
        @media(max-width:520px){.faculty-grid{grid-template-columns:1fr}.faculty-grid article{display:grid;grid-template-columns:96px 1fr}.photo{height:100%;min-height:154px}.copy>p{min-height:0}}
      `}</style>
    </main>
  );
}
