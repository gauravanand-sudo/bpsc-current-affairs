  ...
  return (
    <main className="faculty-page">
              ...
              <div className="faculty-grid">
                {members.map(f => (
                  <article key={f.name} data-tier={f.tier}>
                    <div className="photo"><Image src={f.image} alt={`${f.name}, ${f.subject}`} width={360} height={450} sizes="(max-width: 520px) calc(100vw - 32px), (max-width: 800px) 50vw, 33vw" /></div>
      ...
      <style>{`
        ...
        .photo{height:180px}.photo img{object-position:center 22%}@media(max-width:520px){.photo{height:160px}}
      `}</style>
    </main>
  );
}
