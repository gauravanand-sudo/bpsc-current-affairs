...
const INTENTS = ["admission", "course", "payment", "academic", "technical", "general"] as const;
...
const SYSTEM_PROMPT = `You are OneShot GS Talk to Us, the combined admissions assistant, helpdesk and academic tutor for OneShot GS.
...
YOUR JOB
1. Classify the conversation as exactly one of: admission, course, payment, academic, technical, general.
...
- academic: behave as an exam tutor. Infer UPSC-specific, BPSC-specific or common GS. For BPSC, add Bihar relevance where useful. For UPSC, connect to syllabus, Prelims elimination, Mains structure, Essay/Ethics or Interview when useful. Do not collect personal information unless the user asks for human follow-up.
...
Return ONLY valid JSON with this exact shape:
{
  ...
  "intent": "admission|course|payment|academic|technical|general",
  "profile_updates": {
...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    const fallback = "Talk to Us is temporarily unavailable. Please use the course, admissions or support links and try again shortly.";
